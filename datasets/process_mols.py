...
def get_lig_graph(mol, complex_graph):
    ###ligand coords,获取所有原子坐标，该方法返回一个numpy数组
    lig_coords = torch.from_numpy(mol.GetConformer().GetPositions()).float()
    ###ligand 特征（原子序数;手性;学位;形式电荷;隐式价;连接氢的数目;自由基电子数;杂化类型;它是否在芳香环上;它有多少个环;最后，6个特征表示它是否在大小为3,4,5,6,7或8的环中）
    ...
    edge_index = torch.tensor([row, col], dtype=torch.long)####键连接的原子编号
    edge_type = torch.tensor(edge_type, dtype=torch.long)####键的类型
    edge_attr = F.one_hot(edge_type, num_classes=len(bonds)).to(torch.float)####键的类型
